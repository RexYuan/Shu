...
%TODO add function type alpha to beta
%TODO replace str -> str to key -> value concretely theyre strings
%TODO replace key -> value with row type
%TODO new definite use emph
...
A positive predicate of rows $\pos : (\mathtt{str} \implies \mathtt{str}) \implies \mathtt{bool}$ takes a row and determines if its prediction is positive. For example, $\pos(r_i) := \mathtt{int}(r_i(\text{``score''})) > 7$ means a row's prediction is positive if its score is greater than 7. A positive predicate of model results $\pos : \alpha \implies \mathtt{bool}$ takes a model's result and determines if it is positive. In the simplest case, the model returns a $\mathtt{bool}$, and the positive predicate can just be the idnetity function.
...
%TODO: explain priv pos relationship with Y S
...
%TODO: on a scale of 10, we have 1.5  difference on avg, and it remains to be said that if this is fair, should consult expert by letting them choose epsilon
%TODO: remove epsilon=5
