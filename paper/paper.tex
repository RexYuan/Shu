...
%TODO: clarify the og defs and the role our tools plays
% 1 fairness measures and compas eq odds
% 2 our framework's role and it can do these calcs "and more"
% 3 3rd party is important and with our tool...
% 4 transparency
...
%TODO: fit this in
Including independent and trusted auditors is pivotal for objectivity and accountability; third parties are often further necessary for their specialized expertise in relevant domains. Auditors, equipped with our tool, can thoroughly review AI systems for bias and fairness violations.
...
%TODO: rewrite this. how is our tool transparent
...
A \emph{score predicate} of rows $\sco : \mathtt{row} \implies \alpha$ takes a row and gives its predicted score. Similar to positive predicate, a score predicate of model results $\scom : \alpha \implies \beta$ takes a model's result and gives its score.
...
%TODO: to calculate equal opp we call the function
\begin{align*}
...
\begin{align*}
    \abs{P[Y = 1 | S = 1, \hat{V} = v] - P[Y = 1 | S \neq 1, \hat{V} = v]} \leq \epsilon
\end{align*}
Here we model $Y$ and $S$ as above. As for $\hat{V} = v$, we abstracted $\hat{V}$ with $\calib$ and $v$ with $args$. We have $\hat{V} = v$ if and only if $\calib(v, r_i)$ is true.
\begin{align*}
...
\[
    \abs{P[\hat{Y} = 1 | S = 1, L = l] - P[\hat{Y} = 1 | S \neq 1, L = l]} \leq \epsilon
\]
$\hat{Y}$ and $S$ are modeled as above. As for $L = l$, we abstracted it similarly to the case above by having $L = l$ if and only if $\leg(l)$ is true.
\begin{align*}
...
We will set the unprivileged predicate be to ``African-American'' so that the unprivileged group will be African-American to test if the African-American race is discriminated.
\[
...
\]
On the other hand, when we're taking prediction input from our makeshift model, we simply return its result because it already gives a binary $\mathtt{bool}$ result: $\posm(r_i) := \model(r_i)$
...
%TODO: explain why other measures still work because they're taking the difference
...
\begin{figure*}[h]
    ...
    \begin{minipage}[b]{0.48\linewidth}
        ...
        \subcaption{Privileged Group: African-American}\label{fig:1}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Non-Caucasian}
        \subcaption{Privileged Group: Caucasian}\label{fig:2}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Non-Asian}
        \subcaption{Privileged Group: Asian}\label{fig:2}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Non-Native American}
        \subcaption{Privileged Group: Native American}\label{fig:2}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Non-Hispanic}
        \subcaption{Privileged Group: Hispanic}\label{fig:2}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
...
\begin{figure*}[h]
    ...
    \begin{minipage}[b]{0.48\linewidth}
        ...
        \subcaption{Privileged Group: Greater than 25}\label{fig:1}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
    ...
    \begin{minipage}[b]{0.48\linewidth}
        ...
        \subcaption{Privileged Group: Less than 25 or Greater than 45}\label{fig:2}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
    ...
    \begin{minipage}[b]{0.48\linewidth}
        ...
        \subcaption{Privileged Group: Less than 45}\label{fig:2}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\linewidth}
        \centering
