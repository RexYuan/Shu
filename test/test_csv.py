...
c.equal_calibration(0.2, lambda row:                 row['sex'] == sex, lambda u,l: (lambda row: l <= int(row['decile_score']) and int(row['decile_score']) <= u), (7, 5), lambda row: row['is_recid'] == '1')
...
c.nagative_balance(5, lambda row:                    row['sex'] == sex, lambda row: int(row['decile_score']), lambda row: row['is_recid'] == '1')
